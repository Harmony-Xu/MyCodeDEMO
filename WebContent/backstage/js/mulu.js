$(function (){
	...
	layui.use('table', function(){
	  ...
	  table.render({
	    elem: '#ordergride'
	    ,url: path+'/user/selectAllcatalogue'
	    ,totalRow: true
	    ,loading: true
	    ,toolbar: '#toolbarDemo'
	    	  ,cols: [[     
	    	      ...
	    	      ,{field:'updaloadfilenumbermax', title: '当前目录容量'}
	    	     ,{field:'', title: '目录类型', templet: function(res){
...
function addfileshow(data){
	$("#addfilemodle #catalogueid").val(data.id);//目录id
	$('#addfilemodle').modal('show');
}

function updateshow(data){
	$("#updateModal #id").val(data.id);
	$("#updateModal #cataloguename").val(data.cataloguename);
...
function insert(){
	...
	var flag=true
	/*var updaloadfilenumbermax= $("#insertForm #updaloadfilenumbermax").val();
	if(isNaN(updaloadfilenumbermax)){
		flag=false;
		layer.msg("请输入数字！",{offset:320,icon: 5});		
		return;
	}*/
	var filenumbermax= $("#insertForm #filenumbermax").val();
	if(isNaN(filenumbermax)){
		flag=false;
		layer.msg("请输入数字！",{offset:320,icon: 5});		
		return;
	}
/*	var data= $("#insertForm ").val();
	if(isNaN(updaloadfilenumbermax)){
		flag=false;
		layer.msg("请输入数字！",{offset:320,icon: 5});		
		return;
		 ...
		 $.ajax({
				...
				success: function (req){
					if (req){
						...
						layer.msg("存在同名目录，操作失败！",{offset:320,icon: 2});
					}
				},
				error: function(req){
					layer.msg("请联系管理员！",{offset:320,icon: 5});
